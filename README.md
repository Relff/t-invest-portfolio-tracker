...
Для каждого файла из репозитория создайте скрипт в Apps Script:

| Файл | Назначение |
|---|---|
| `tinvest.js` | Ядро: подключение к T-Invest API |
| `dashboard.js` | Главный файл: конфиг, дашборд, ребалансировка |
| `income.js` | Лист «Ожидаемый доход» |
| `calendar.js` | Лист «Календарь выплат» |
| `history.js` | Лист «История операций» |
| `charts.js` | Диаграммы на Дашборде |
...
|---|---|---|
| Глубина истории | `history.js` | `HISTORY_MONTHS = 12` |
| Топ-N облигаций в доходе | `income.js` | `BOND_TOP_N = 15` |
| Пороги цветовой подсветки | `dashboard.js` | `THR = { OK: 1.5, WARN: 3.0 }` |
